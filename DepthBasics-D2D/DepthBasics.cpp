...
int CDepthBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    wc.lpszClassName = L"DepthBasicsAppDlgWndClass";//need change
...
void CDepthBasics::Update()
{
	...
	{
	hr2 = m_pColorFrameReader->AcquireLatestFrame(&pColorFrame);
	}
    if (SUCCEEDED(hr))
    {
        INT64 nTime1 = 0;
		...
		IFrameDescription* pFrameDescription2 = NULL;
        int nWidth1 = 0;
        int nHeight1 = 0;
		int nWidth2 = 0;
		int nHeight2 = 0;
        ...
        UINT nBufferSize1 = 0;
		UINT nBufferSize2 = 0;
        UINT16 *pBuffer1 = NULL;
		RGBQUAD *pBuffer2 = NULL;

        hr = pDepthFrame->get_RelativeTime(&nTime1);
		if (SUCCEEDED(hr2))
		{
		hr2 = pColorFrame->get_RelativeTime(&nTime2);
		}
		...
		if (SUCCEEDED(hr))
		{
			hr = pDepthFrame->get_FrameDescription(&pFrameDescription1);
		}
		if (SUCCEEDED(hr2))
		{
			hr = pColorFrame->get_FrameDescription(&pFrameDescription2);
        }

		if (SUCCEEDED(hr))
		{
			hr = pFrameDescription1->get_Width(&nWidth1);
		}
		if (SUCCEEDED(hr2))
		{
			hr2 = pFrameDescription2->get_Width(&nWidth2);
        }

		if (SUCCEEDED(hr))
		{
			hr = pFrameDescription1->get_Height(&nHeight1);
		}
		if (SUCCEEDED(hr2))
		{
			hr2 = pFrameDescription2->get_Height(&nHeight2);
        }
        ...
        if (SUCCEEDED(hr))
        {
            hr = pDepthFrame->AccessUnderlyingBuffer(&nBufferSize1, &pBuffer1);            
        }
		if (SUCCEEDED(hr2))
		{
			if (imageFormat == ColorImageFormat_Bgra)
			{
				hr2 = pColorFrame->AccessRawUnderlyingBuffer(&nBufferSize2, reinterpret_cast<BYTE**>(&pBuffer2));
			}
			else if (m_pColorRGBX)
			{
				pBuffer2 = m_pColorRGBX;
				nBufferSize2 = cColorWidth * cColorHeight * sizeof(RGBQUAD);
				hr2 = pColorFrame->CopyConvertedFrameDataToArray(nBufferSize2, reinterpret_cast<BYTE*>(pBuffer2), ColorImageFormat_Bgra);
			}
			else
			{
        ...
        if (SUCCEEDED(hr))
        {
            ProcessDepth(nTime1, pBuffer1, nWidth1, nHeight1, nDepthMinReliableDistance, nDepthMaxDistance);

        }
		if (SUCCEEDED(hr2))
		{
			ProcessColor(nTime2, pBuffer2, nWidth2, nHeight2);
		}

        SafeRelease(pFrameDescription1);
		SafeRelease(pFrameDescription2);
    }
...
LRESULT CALLBACK CDepthBasics::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_COMMAND:
            ...
            break;
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    //Should do something when we only have .xef file instead of real-time data
    }
...
void CDepthBasics::ProcessDepth(INT64 nTime, const UINT16* pBuffer, int nWidth, int nHeight, USHORT nMinDepth, USHORT nMaxDepth)
{
	...
	if (frame_count == 0)
	{
		...
		itoa(local->tm_year, intStr, 10);
		
		char* buf=new char[20];
		strftime(buf, 20, "%Y.%m.%d %H-%M-%S", local);
		timeforfile = string(buf);
		//timeforfile = "1";
	}
	string temp = string("E:/kinect_test/recording_frame/") + timeforfile;
	...
	Mat img(nHeight, nWidth, CV_16SC1, (void *)pBuffer);
	char *intStr=new char[100];
	_itoa_s(frame_count,intStr,100,10);
	string num = string(intStr);
	imshow("depth", img);
	imwrite("E:/kinect_test/recording_frame/"+timeforfile+"/depth_"+num+".png", img);
	waitKey(1);
	frame_count++;
    if (m_hWnd)
    {
...
void CDepthBasics::ProcessColor(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight)
{
	...
	RGBQUAD* pBufferEnd = pBuffer + (nWidth*nHeight);
	while (pBuffer < pBufferEnd)
	{
